//go:build amnezia

/* SPDX-License-Identifier: MIT
 *
 * Copyright (C) 2019-2025.
 */

package tunnel

import (
	"bytes"
	"fmt"
	"net"
	"log"
	"os"
	"runtime"
	"time"

	"github.com/amnezia-vpn/amneziawg-go/conn"
	"github.com/amnezia-vpn/amneziawg-go/device"
	"github.com/amnezia-vpn/amneziawg-go/ipc"
	"github.com/amnezia-vpn/amneziawg-go/tun"
	"golang.org/x/sys/windows"
	"golang.org/x/sys/windows/svc"
	"golang.org/x/sys/windows/svc/mgr"
	"golang.zx2c4.com/wireguard/windows/conf"
	"golang.zx2c4.com/wireguard/windows/elevate"
	"golang.zx2c4.com/wireguard/windows/ringlogger"
	"golang.zx2c4.com/wireguard/windows/services"
	"golang.zx2c4.com/wireguard/windows/tunnel/winipcfg"
)

type userspaceTunnelService struct {
	Path string
}

func (service *userspaceTunnelService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (svcSpecificEC bool, exitCode uint32) {
	changes <- svc.Status{State: svc.StartPending}

	var dev *device.Device
	var uapiListener net.Listener
	var nativeTun *tun.NativeTun
	var config *conf.Config
	var luid winipcfg.LUID
	var err error
	serviceError := services.ErrorSuccess

	defer func() {
		svcSpecificEC, exitCode = services.DetermineErrorCode(err, serviceError)
		logErr := services.CombineErrors(err, serviceError)
		if logErr != nil {
			log.Println(logErr)
		}
		changes <- svc.Status{State: svc.StopPending}

		stopIt := make(chan bool, 1)
		go func() {
			t := time.NewTicker(time.Second * 30)
			for {
				select {
				case <-t.C:
					t.Stop()
					buf := make([]byte, 1024)
					for {
						n := runtime.Stack(buf, true)
						if n < len(buf) {
							buf = buf[:n]
							break
						}
						buf = make([]byte, 2*len(buf))
					}
					lines := bytes.Split(buf, []byte{'\n'})
					log.Println("Failed to shutdown after 30 seconds. Probably dead locked. Printing stack and killing.")
					for _, line := range lines {
						if len(bytes.TrimSpace(line)) > 0 {
							log.Println(string(line))
						}
					}
					os.Exit(777)
					return
				case <-stopIt:
					t.Stop()
					return
				}
			}
		}()

		if logErr == nil && dev != nil && config != nil {
			logErr = runScriptCommand(config.Interface.PreDown, config.Name)
		}
		if uapiListener != nil {
			uapiListener.Close()
		}
		if dev != nil {
			dev.Close()
		}
		if logErr == nil && config != nil {
			_ = runScriptCommand(config.Interface.PostDown, config.Name)
		}
		stopIt <- true
		log.Println("Shutting down")
	}()

	var logFile string
	logFile, err = conf.LogFile(true)
	if err != nil {
		serviceError = services.ErrorRingloggerOpen
		return
	}
	err = ringlogger.InitGlobalLogger(logFile, "TUN")
	if err != nil {
		serviceError = services.ErrorRingloggerOpen
		return
	}

	config, err = conf.LoadFromPath(service.Path)
	if err != nil {
		serviceError = services.ErrorLoadConfiguration
		return
	}
	config.DeduplicateNetworkEntries()

	log.SetPrefix(fmt.Sprintf("[%s] ", config.Name))

	services.PrintStarting()

	if services.StartedAtBoot() {
		if m, err := mgr.Connect(); err == nil {
			if lockStatus, err := m.LockStatus(); err == nil && lockStatus.IsLocked {
				log.Printf("SCM locked for %v by %s, marking service as started", lockStatus.Age, lockStatus.Owner)
				changes <- svc.Status{State: svc.Running}
			}
			m.Disconnect()
		}
	}

	log.Println("Resolving DNS names and building UAPI")
	uapiConf, err := config.ToUAPI()
	if err != nil {
		serviceError = services.ErrorDNSLookup
		return
	}

	log.Println("Creating Wintun interface")
	var wintunDev tun.Device
	for i := 0; i < 5; i++ {
		if i > 0 {
			time.Sleep(time.Second)
			log.Printf("Retrying Wintun creation after failure because system just booted (T+%v): %v", windows.DurationSinceBoot(), err)
		}
		wintunDev, err = tun.CreateTUNWithRequestedGUID(config.Name, deterministicGUID(config), 0)
		if err == nil || windows.DurationSinceBoot() > time.Minute*4 {
			break
		}
	}
	if err != nil {
		serviceError = services.ErrorCreateNetworkAdapter
		return
	}
	nativeTun = wintunDev.(*tun.NativeTun)
	luid = winipcfg.LUID(nativeTun.LUID())
	if ver, err := nativeTun.RunningVersion(); err == nil {
		log.Printf("Using Wintun/%d.%d", (ver>>16)&0xffff, ver&0xffff)
	}

	err = runScriptCommand(config.Interface.PreUp, config.Name)
	if err != nil {
		serviceError = services.ErrorRunScript
		return
	}

	log.Println("Enabling firewall rules")
	err = enableFirewall(config, luid)
	if err != nil {
		serviceError = services.ErrorFirewall
		return
	}

	log.Println("Dropping privileges")
	err = elevate.DropAllPrivileges(true)
	if err != nil {
		serviceError = services.ErrorDropPrivileges
		return
	}

	log.Println("Creating userspace interface instance")
	bind := conn.NewDefaultBind()
	dev = device.NewDevice(wintunDev, bind, &device.Logger{log.Printf, log.Printf})

	log.Println("Setting interface configuration (UAPI)")
	uapiListener, err = ipc.UAPIListen(config.Name)
	if err != nil {
		serviceError = services.ErrorUAPIListen
		return
	}
	if err = dev.IpcSet(uapiConf); err != nil {
		serviceError = services.ErrorDeviceSetConfig
		return
	}

	// Bring device up and configure IP/DNS using our existing helpers
	dev.Up()
	if err = configureInterface(windows.AF_INET, config, luid); err != nil {
		serviceError = services.ErrorSetNetConfig
		return
	}
	if err = configureInterface(windows.AF_INET6, config, luid); err != nil {
		serviceError = services.ErrorSetNetConfig
		return
	}

	log.Println("Listening for UAPI requests")
	go func() {
		for {
			c, err := uapiListener.Accept()
			if err != nil {
				continue
			}
			go dev.IpcHandle(c)
		}
	}()

	err = runScriptCommand(config.Interface.PostUp, config.Name)
	if err != nil {
		serviceError = services.ErrorRunScript
		return
	}

	changes <- svc.Status{State: svc.Running, Accepts: svc.AcceptStop | svc.AcceptShutdown}
	log.Println("Startup complete")

	for {
		select {
		case c := <-r:
			switch c.Cmd {
			case svc.Stop, svc.Shutdown:
				return
			case svc.Interrogate:
				changes <- c.CurrentStatus
			default:
				log.Printf("Unexpected service control request #%d\n", c)
			}
		}
	}
}
